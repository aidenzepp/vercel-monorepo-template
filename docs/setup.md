...
Vercel Production deployment `dpl_DFcdC5HqbqrcMh1EN2K8dDC1wM7e` is Ready at `https://app.templ8.dev`. The approved tester completed Google sign-in, signed out, and resumed the same account through the production callback. Preview deployment `dpl_661kX8uYFFdYASvzQdtp8ANSWyam` completed the OAuth Proxy flow through that sole production callback and returned an authenticated session to `https://templ8-4ifmj2zyn-superargentic.vercel.app`. Local development loaded temporary Development-scoped values, sent Google through the same production callback, and returned an authenticated session to `http://localhost:3000`; the temporary environment file and deployment snapshot were then deleted. These results prove the Google OAuth paths only and do not replace the separate database and migration gates above. Testing status remains restricted to listed testers and is not a public-launch configuration.
