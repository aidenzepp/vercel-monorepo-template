...
`app/api/files/route.ts` exposes private user media through the Files SDK's authenticated gateway. `GET` proxies authorized downloads, while `POST` handles only upload metadata and completion verification for explicitly registered namespaces. Avatar uploads begin after identity validation succeeds; the browser sends image bytes directly to a short-lived signed Blob target, so the application route never receives the file body. Every object remains scoped under the current permanent user's `users/<id>/` prefix, and the stored `user.image` value is a stable application URL rather than a provider URL. The Blob contract requires the stable store identifier and webhook public key. Its long-lived read-write token remains optional and absent from the default setup because Vercel's Blob SDK resolves short-lived OIDC identity at runtime; application code does not read or validate the raw OIDC token.
