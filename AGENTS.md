...
- `apps/web` is the authenticated product. It composes its application environment contract and owns its Neon connection, Drizzle schema and migrations, Better Auth runtime configuration, and auth API route.
...
- `packages/t3-env` owns reusable server environment contracts for Better Auth, Resend, and Vercel Blob. Applications compose them through direct config subpaths alongside T3 Env's upstream provider presets; the package must not own runtime integrations or application-only variables.
...
Ship foundation skills that apply broadly across the checked-in workspace. The repo-owned foundation set is `async-boundaries`, `component-decomposition`, `function-decomposition`, `document-code`, and `conventional-commits`.
